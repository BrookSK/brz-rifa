<?php
/**
 * VERSÃO TRABALHANDO - Sistema BRZ Rifa
...
// Configurações inline - SEM CLASSE CONFIG
...
function getDBConnection() {
    try {
        ...
        return null; // Retorna null se não conectar
    }
}
...
function showHome() {
    ...
    if ($db) {
        try {
            $stmt = $db->query("SELECT COUNT(*) as count FROM raffles WHERE status = 'active'");
            $stats['active_raffles'] = $stmt->fetch()['count'];
            
            $stmt = $db->query("SELECT COUNT(DISTINCT participant_cpf) as count FROM raffle_numbers WHERE status = 'paid'");
            $stats['total_participants'] = $stmt->fetch()['count'];
            
            $stmt = $db->query("SELECT COALESCE(SUM(payment_amount), 0) as total FROM raffle_numbers WHERE status = 'paid'");
            $stats['total_revenue'] = $stmt->fetch()['total'];
            
            $stmt = $db->query("SELECT COUNT(*) as count FROM system_alerts WHERE read_at IS NULL");
